...
class SimpleConvNet:
    ...
    def __init__(self, input_dim=(3, 64, 64), output_size=8,use_dropout=True, dropout_ratio=0.5):
        ...
        H_conv3 = (H_pool2 + 2 * 1 - 3) // 1 + 1  # 16 -> 16
        H_pool3 = H_conv3 // 2  # 16 -> 8

        W_conv1 = (W + 2 * 1 - 3) // 1 + 1  # 64 -> 64
        W_pool1 = W_conv1 // 2  # 64 -> 32
        W_conv2 = (W_pool1 + 2 * 1 - 3) // 1 + 1  # 32 -> 32
        W_pool2 = W_conv2 // 2  # 32 -> 16
        W_conv3 = (W_pool2 + 2 * 1 - 3) // 1 + 1  # 16 -> 16
        W_pool3 = W_conv3 // 2  # 16 -> 8

        fc_input_size = 48 * H_pool3 * W_pool3  # 48 × 8 × 8 = 3072

        # 全连接层也使用He初始化
        scale3 = np.sqrt(2.0 / fc_input_size)
        self.params['W3'] = scale3 * np.random.randn(fc_input_size, 256)  # 256 -> 128
        self.params['b3'] = np.zeros(256)  # 256 -> 128

        scale4 = np.sqrt(2.0 / 256)  # 256 -> 128
        self.params['W4'] = scale4 * np.random.randn(256, output_size)  # 256 -> 128
        ...
        self.params['gamma_bn3_conv'] = np.ones(48)  # Conv3后的BN
        self.params['beta_bn3_conv'] = np.zeros(48)
        self.params['gamma_bn3'] = np.ones(256)  # Affine1后的BN，256 -> 128
        self.params['beta_bn3'] = np.zeros(256)  # 256 -> 128
