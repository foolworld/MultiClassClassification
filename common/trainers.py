...
class Trainer:
    ...
    def __init__(self, network, x_train, t_train, x_val, t_val,learning_rate=0.01,batch_size=32, epochs=20, verbose=True, use_augmentation=True):
        """
        初始化训练器
    ...
    def train(self):
        ...
        for i in range(self.max_iter):
            ...
            if (i + 1) % self.iter_per_epoch == 0:
                ...
                if (self.augmentation_enabled and not self.augmentation_triggered and train_acc > self.trigger_acc_threshold):
                    ...
                    if self.high_acc_epochs >= self.trigger_stable_epochs:
                        ...
                        if self.verbose:
                            print(f"🚀 数据增强已触发! (训练准确率连续{self.trigger_stable_epochs}轮 > {self.trigger_acc_threshold})")
                ...
                if self.augmentation_triggered:
                    # 线性增加增强强度，从0.2到1.0
                    self.augmentation_progress = min((self.current_epoch - self.trigger_stable_epochs) / 40, 1.0)
