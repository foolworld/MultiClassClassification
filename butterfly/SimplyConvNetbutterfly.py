# simple_convnet_butterfly.py
import numpy as np
from collections import OrderedDict
from common.layers import Convolution, Affine, Pooling, Relu, SoftmaxWithLoss, Dropout


class SimpleConvNetButterfly:

    def __init__(self, input_dim=(3, 56, 56), output_size=100, use_dropout=True, dropout_ratio=0.5,
        ...
        self.params['W1'] = scale1 * np.random.randn(32, C, 3, 3)
        self.params['b1'] = np.zeros(32)

        # 卷积层2: 输入通道32, 卷积核3x3
        scale2 = np.sqrt(2.0 / (32 * 3 * 3))
        self.params['W2'] = scale2 * np.random.randn(64, 32, 3, 3)
        self.params['b2'] = np.zeros(64)

        # 新增卷积层3: 输入通道64, 卷积核3x3
        scale3_conv = np.sqrt(2.0 / (64 * 3 * 3))
        self.params['W3_conv'] = scale3_conv * np.random.randn(128, 64, 3, 3)
        self.params['b3_conv'] = np.zeros(128)

        # 计算特征图尺寸
        H_conv1 = (H + 2 * 1 - 3) // 1 + 1  # 56 -> 56
        H_pool1 = H_conv1 // 2  # 56 -> 28
        H_conv2 = (H_pool1 + 2 * 1 - 3) // 1 + 1  # 28 -> 28
        H_pool2 = H_conv2 // 2  # 28 -> 14
        H_conv3 = (H_pool2 + 2 * 1 - 3) // 1 + 1  # 14 -> 14 (新增)
        H_pool3 = H_conv3 // 2  # 14 -> 7 (新增)

        W_conv1 = (W + 2 * 1 - 3) // 1 + 1  # 56 -> 56
        W_pool1 = W_conv1 // 2  # 56 -> 28
        W_conv2 = (W_pool1 + 2 * 1 - 3) // 1 + 1  # 28 -> 28
        W_pool2 = W_conv2 // 2  # 28 -> 14
        W_conv3 = (W_pool2 + 2 * 1 - 3) // 1 + 1  # 14 -> 14 (新增)
        W_pool3 = W_conv3 // 2  # 14 -> 7 (新增)

        # 根据是否使用全局平均池化调整全连接层输入大小
        if self.use_global_pooling:
            fc_input_size = 128  # 全局平均池化后，每个通道一个值
        else:
            fc_input_size = 128 * H_pool3 * W_pool3  # 128 × 7 × 7 = 6272
        ...
        # 添加BatchNorm参数
        self.params['gamma_bn1'] = np.ones(32)  # Conv1后的BN
        self.params['beta_bn1'] = np.zeros(32)
        self.params['gamma_bn2'] = np.ones(64)  # Conv2后的BN
        self.params['beta_bn2'] = np.zeros(64)
        self.params['gamma_bn3_conv'] = np.ones(128)  # 新增Conv3后的BN
        self.params['beta_bn3_conv'] = np.zeros(128)
...
class BatchNormalization:
    ...
    def forward(self, x, train_flg=True):  # 添加forward方法
